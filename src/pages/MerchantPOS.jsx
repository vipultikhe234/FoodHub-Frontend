        ...
        <div className="space-y-12 pb-20 font-sans max-w-[1240px] mx-auto min-h-screen pt-6 text-left">
            {loading ? (
                ...
                <>
                                ...
                                {activeOperationOrders.map((order, idx) => {
                                    ...
                                    return (
                                        <motion.div 
                                            ...
                                            className="bg-zinc-900 border-2 border-white dark:border-zinc-800 rounded-[48px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.4)] p-10 overflow-hidden relative group"
                                        >
                                            <div className="flex flex-col lg:flex-row justify-between gap-8 items-start">
                                                <div className="flex-1 w-full">
                                                    ...
                                                    <div className="flex items-center gap-5 mb-8">
                                                        <div className="bg-white/10 w-14 h-14 rounded-2xl flex items-center justify-center text-white border border-white/10 shrink-0">
                                                            <Package size={24} />
                                                        </div>
                                                        <div>
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-[0.2em] flex items-center gap-2 border ${order.payment_method?.toLowerCase() === 'cod' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>
                                                    ...
                                                    <div className="relative mt-2 mb-8 px-4">
                                                        <div className="absolute top-1/2 left-4 right-4 h-1 bg-white/5 -translate-y-1/2 rounded-full" />
                                                        <div className="absolute top-1/2 left-4 h-1 bg-blue-500 -translate-y-1/2 rounded-full shadow-[0_0_15px_rgba(59,130,246,0.6)] transition-all duration-700" style={{ width: `${(currentStepIndex / (steps.length - 1)) * 100}%` }} />
                                                        ...
                                                        <div className="relative flex justify-between">
                                                            {steps.slice(0, 4).map((s, i) => {
                                                                ...
                                                                return (
                                                                    <div key={s} className="flex flex-col items-center">
                                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center z-10 transition-all duration-500 ${active ? 'bg-blue-600 text-white scale-110 shadow-xl' : 'bg-zinc-800 border-2 border-zinc-700 text-zinc-600'}`}>
                                                                            {i < currentStepIndex ? <Package size={16} /> : active ? <Activity size={16} /> : <div className="w-1 h-1 rounded-full bg-zinc-600" />}
                                                                        </div>
                                                            ...
                                                            <div className="flex flex-col items-center">
                                                                <div className="w-10 h-10 rounded-full bg-zinc-800 border-2 border-zinc-700 opacity-20 flex items-center justify-center">
                                                                    <div className="w-3 h-3 border-2 border-dashed border-zinc-400 rounded" />
                                                                </div>
                                                ...
                                                <div className="w-full lg:w-[360px] bg-white/5 rounded-[36px] p-7 border border-white/10 flex flex-col h-full">
                                                        ...
                                                        {order.items?.map((item, i) => {
                                                            ...
                                                            return (
                                                                <button 
                                                                    ...
                                                                    className={`w-full flex justify-between items-center p-3 rounded-xl border transition-all ${isPacked ? 'bg-emerald-500/10 border-emerald-500/20 opacity-50 scale-[0.98]' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}
                                                                >
                                                    ...
                                                    <button 
                                                        ...
                                                        className={`w-full h-13 py-4 bg-white text-zinc-900 rounded-[22px] text-[10px] font-black uppercase tracking-[0.2em] shadow-xl transition-all flex items-center justify-center gap-2 ${processing ? 'opacity-50 cursor-not-allowed' : 'hover:scale-[1.02] active:scale-[0.98]'}`}
                                                    >
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-[44px] border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                        <div className="flex items-center justify-between px-8 py-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-800/5">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-white dark:bg-zinc-800 rounded-xl shadow-sm italic font-black text-[10px] border border-zinc-100 dark:border-zinc-800"> QUEUE </div>
                                <h3 className="text-lg font-black text-zinc-900 dark:text-white uppercase italic tracking-tighter">Pending Deployments</h3>
                            </div>
                            <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[8px] font-black uppercase tracking-widest border border-blue-100 italic">
                                    ...
                                    <div key={order.id} className="grid grid-cols-1 md:grid-cols-5 gap-4 px-8 py-6 items-center hover:bg-zinc-50/40 dark:hover:bg-zinc-800/10 transition-all group">
                                        <div className="col-span-2">
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <div className={`px-2 py-0.5 rounded-md text-[7px] font-black uppercase tracking-widest border ${order.payment_method?.toLowerCase() === 'cod' ? 'bg-amber-50 text-amber-500 border-amber-100' : 'bg-emerald-50 text-emerald-500 border-emerald-100'}`}>
                                                    {order.payment_method || 'COD'}
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex justify-end">
                                            <button 
                                                ...
                                                className={`h-11 px-8 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-[9px] font-black uppercase tracking-[0.15em] shadow-lg transition-all flex items-center gap-2 ${isProcessing ? 'opacity-50' : 'hover:scale-105 active:scale-95'}`}
                                            >
