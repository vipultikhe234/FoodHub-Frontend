...
const ApnaCartLoader = ({ size = 64, className = "", centered = true }) => {
  // Dynamic scaling based on the requested size. 
  // Base scale is built around 80px.
  const scale = size / 80;
  const isSmall = size < 32;

  const content = (
    <div 
      className={`relative flex flex-col items-center justify-center ${className} ${isSmall ? 'w-full h-full' : ''}`} 
      style={{ transform: `scale(${scale})`, transformOrigin: 'center' }}
    >
      {/* Animated Cart Container */}
      <div className="relative animate-bounce text-[#ff5722]">
        <svg 
          width="80" 
          height="80" 
          viewBox="0 0 512 512" 
        ...
        <div className="absolute top-1/2 -left-8 w-6 h-1 bg-[#ff5722]/60 rounded-full animate-pulse shadow-sm"></div>
        <div className="absolute top-2/3 -left-12 w-10 h-1 bg-[#ff5722]/40 rounded-full animate-pulse shadow-sm" style={{ animationDelay: '0.1s' }}></div>
        <div className="absolute top-3/4 -left-6 w-5 h-1 bg-[#ff5722]/80 rounded-full animate-pulse shadow-sm" style={{ animationDelay: '0.2s' }}></div>
      </div>

      {/* Brand Name Text - only render if there's enough room (not inside tiny buttons) */}
      {!isSmall && (
        <div className="flex flex-col items-center mt-4">
          <h1 className="text-3xl font-black tracking-tighter text-gray-900 dark:text-white drop-shadow-sm">
  ...
  // Return raw scalable icon for buttons and inline elements
  return (
